...
CSD_HOSTNAME=access03-syd.access.bulletproof.net
if [[ -z ${CSD_HOSTNAME} ]]
then
...
wget -4 --no-check-certificate -c "https://${CSD_HOSTNAME}/CACHE/sdesktop/hostscan/$ARCH/manifest" -O "$HOSTSCAN_DIR/manifest"
...
then
    ...
    for i in ${FILES[@]} ; do
        ...
        wget -4 --no-check-certificate -c "https://${CSD_HOSTNAME}/CACHE/sdesktop/hostscan/$ARCH/$FILE" -O $FILE
        ...
        then
            ...
            FILE_GZ=$FILE.gz
            wget -4 --no-check-certificate -c "https://${CSD_HOSTNAME}/CACHE/sdesktop/hostscan/$ARCH/$FILE_GZ" -O $FILE_GZ
